...
\
    ...
    uint128_t by_status_type () const { return (uint128_t(type.value) << 64) + proposal_id; } \
    uint128_t by_parent_status () const { return (uint128_t(parent) << 64) + status.value; } \
    uint128_t by_parent_current_phase () const { return (uint128_t(parent) << 64) + current_phase.value; } \
    uint128_t by_parent_type () const { return (uint128_t(parent) << 64) + type.value; } \
\
    EOSLIB_SERIALIZE( \
      proposal_table, \
