...
describe('Tests for amendment proposals', async function () {
  ...
  it('An account can not create an amendment when main proposal is not on debate phase', async function () {

    // Arrange
    const proposal_amendment = await ProposalsFactory.createAmendmentWithDefaults({})

    await TokenUtil.issue({ amount: minstake, issuer: token, contract: contracts.token })
    await contracts.token.transfer(token, proposal_amendment.params.creator, minstake, 'initial supply for creating a proposal', { authorization: `${token}@active` })

    // Act
    try {
      await contracts.proposals.create(proposal_amendment.getActionParams(), { authorization: `${proposal_amendment.params.creator}@active` })
    } catch (err) {
      error = err
    }
    ...
    assertError({
      error,
      textInside: 'Amendment proposal can be only created when main proposal is on debate phase',
      verbose: false
    })
  ...
  it('An account can create an amendment when main proposal is on debate phase', async function () {
  ...
  it('An account moves his amendment to discuss (main proposal get an awaiting)', async function () {
    ...
    // Act
    await contracts.proposals.move(2, { authorization: `${proposal_amendment.params.creator}@active` })
...
it('Main proposal can not be moved while there is an active amendment', async function () {
