...
describe("Proposals update wrong auth", () => {
  beforeEach(async () => {
    blockchain.resetTables()
    await initContracts()
  })

  it("update fails when not signed by creator", async () => {
    const creator = "alice"
    ...
    let threw = false
    try {
      await prop("update", { args: [{ first: "proposal_id", second: ["int64", id] }] }, `bob@active`)
    } catch (e) {
      threw = true
      const msg = String(e)
